...
def test_create_text_capture(client, db_session, auth_token):
    ...
    response = client.post(
        ...
        json={"capture_type": "text", "raw_text": "Buy groceries tomorrow at 5pm"},
...
def test_create_voice_capture(client, db_session, auth_token):
    ...
    response = client.post(
        "/api/v1/captures/captures",
        json={"capture_type": "voice", "audio_data": audio_base64},
        headers={"Authorization": f"Bearer {auth_token}"},
    )
...
def test_create_image_capture(client, db_session, auth_token):
    ...
    response = client.post(
        "/api/v1/captures/captures",
        json={"capture_type": "image", "image_data": image_base64},
        headers={"Authorization": f"Bearer {auth_token}"},
    )
...
def test_create_capture_without_auth(client):
    ...
    response = client.post(
        "/api/v1/captures/captures",
        json={"capture_type": "text", "raw_text": "Test capture"},
    )
...
def test_list_captures(client, db_session, auth_token):
    ...
    client.post(
        "/api/v1/captures/captures",
        json={"capture_type": "text", "raw_text": "Test capture 1"},
        headers={"Authorization": f"Bearer {auth_token}"},
    )

    # List captures
    response = client.get(
        "/api/v1/captures/captures", headers={"Authorization": f"Bearer {auth_token}"}
    )
...
def test_input_validation_text_too_long(client, db_session, auth_token):
    ...
    response = client.post(
        "/api/v1/captures/captures",
        json={"capture_type": "text", "raw_text": long_text},
        headers={"Authorization": f"Bearer {auth_token}"},
    )
...
def test_rate_limiting(client):
    ...
    # Make many requests rapidly to root endpoint (not /health which is exempt)
    responses = []
