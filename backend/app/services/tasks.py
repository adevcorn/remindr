    ...
    try:
        ...
        if error:
            if retry_count < settings.SYNC_RETRY_MAX_ATTEMPTS:
                ...
                db.commit()
                logger.error(f"Draft {draft_id} sync failed after {retry_count} retries: {error}")
                return {
                    "draft_id": draft_id,
                    "status": "error",
                    "error": error,
        ...
        logger.info(f"Draft {draft_id} synced successfully (task_id={task_id}, event_id={event_id})")
...
async def process_offline_queue(user_id: str, user_credentials: dict) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Processing offline queue for user {user_id}: {len(draft_ids)} drafts")
        ...
        for draft_id in draft_ids:
            ...
            asyncio.create_task(
                sync_draft_to_google_with_retry(draft_id, user_credentials, retry_count=0)
            )
